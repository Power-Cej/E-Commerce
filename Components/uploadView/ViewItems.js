    ...
    return (
        <SafeAreaView style={styles.mainScreen}>
            <ScrollView style={{ flexGrow: 1, flexDirection: 'column', width: '100%', padding: 20 }}>
                <Text style={{ margin: 5, fontSize: 15, color: 'blue' }}> {itemNumber} products found</Text>

                {/* display fetch data here!  */}
                {itemNumber > 0 ? (
                    callData.map((items, index) => (
                        <View key={index} style={{ flexDirection: 'row', height: 100, alignItems: 'center', marginVertical: 10, }}>
                            {/* Item Image  */}
